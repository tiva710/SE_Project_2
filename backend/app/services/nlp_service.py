...
# ---- Module-level NER helper (place at bottom, after all classes) ----
_ner_engine = None

def run_ner_to_neo4j(text: str):
    """
    Run requirements NER and return Neo4j-ready dict.
    Callable from routes_transcribe.
    """
    global _ner_engine
    if _ner_engine is None:
        _ner_engine = RequirementsNERToNeo4j(use_spacy=True)
    return _ner_engine.process_text(text)
